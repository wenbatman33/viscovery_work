...
function savingLayer() {
  var notifyItem = document.createElement("div");
  var textMSG = document.createTextNode("Video Saving");  
  notifyItem.appendChild(textMSG);
  document.getElementById("notifyCorner").appendChild(notifyItem);
  // removeNotifyItem();
  notifyItem.style.display = "block";
  notifyItem.style.width = "200px";
  notifyItem.style.height = "40px";
  notifyItem.style.lineHeight = "40px";
  notifyItem.style.margin = "5px";
  notifyItem.style.color = "#fff";
  notifyItem.style.opacity = "1";
  notifyItem.style.fontSize = "14px";
  notifyItem.style.textAlign = "center";
  notifyItem.style.borderRadius= "8px";
  notifyItem.style.background = "-webkit-linear-gradient(8deg, #44A3D9 0%, #2A2FBF 100%)";
  notifyItem.style.background = "-moz-linear-gradient(8deg, #44A3D9 0%, #2A2FBF 100%)";
  notifyItem.style.background = "-ms-linear-gradient(8deg, #44A3D9 0%, #2A2FBF 100%)";
  notifyItem.style.boxShadow = "0 2px 4px 0 rgba(0, 0, 0, 0.50)";
  notifyItem.style.transition = "all .3s";
  setTimeout(removeNotifyItem, 5000);
}
  ...
  SaveVideoToServer: function (e, t, i) {
    ...
    o.open("POST", n, !0), o.setRequestHeader("authorization", "Token " + i.token), o.onreadystatechange = function () {
      if (o.DONE === o.readyState)
        if (200 == o.status) {
          var e = JSON.parse(o.responseText);
          e && void 0 !== e.isSuccess && (e.isSuccess ? pinstream.SnedMsg({
            msg: "Success!",
            type: 2
          }, t) : "Exist" === e.msg ? pinstream.SnedMsg({
            msg: "existed!",
            type: 4
          }, t) : pinstream.SnedMsg({
            msg: e.msg,
        ...
        } else 400 == o.status ? pinstream.RedirectLogin() : (pinstream.SnedMsg({
          msg: "Fail!",
          type: 3
        }, t), console.info(o.responseText))
    };
    var r = new FormData;
